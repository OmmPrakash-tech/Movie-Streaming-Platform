...
public class FileUploadServiceImpl implements FileUploadService {
    ...
    public void init() {
        ...
        try {
            ...
            throw new RuntimeException(
                    "Could not create upload directories", ex);
        }
    }
    ...
    public ResponseEntity<Resource> serveVideo(String uuid, String rangeHeader) {
        ...
        try {
            Path filePath =
                    FileHandlerUtil.findFileByUuid(videoStorageLocation, uuid);

            Resource resource =
                    FileHandlerUtil.createFullResource(filePath);

            String filename = resource.getFilename();
            String contentType =
                    FileHandlerUtil.detectVideoContentType(filename);
    ...
    private ResponseEntity<Resource> buildPartialVideoResponse(
        ...
        long[] range =
                FileHandlerUtil.parseRangeHeader(rangeHeader, fileLength);
        ...
        Resource rangeResource =
                FileHandlerUtil.createRangeResource(
                        filePath,
                        rangeStart,
                        contentLength);
